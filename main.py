...
from keymap import load_controls  # <-- NEW
...
# Players (load from keys.json)
p1_controls, p2_controls = load_controls()  # <-- NEW
...
def spawn_banana_if_needed():
    ...
    # ground floor banana
    banana_pickups.add(BananaPickup(x, y_bottom=GROUND_Y))
...
while True:
    ...
    if game_active:
        ...
        # Projectile → player hits (direct damage handled INSIDE proj.on_hit)
        ...
        # Banana icons (show when player has one)
