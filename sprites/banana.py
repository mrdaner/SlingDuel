...
class BananaPickup(pygame.sprite.Sprite):
    """A stationary banana that sits on the ground until a player picks it up."""
...
class Banana(Throwable):
    ...
    def _land_on_surface(self, platforms):
        """Snap to ground or platform if intersecting, return True if landed."""
        # Ground check
    ...
    def update(self, platforms=None, players=None):
        if self.state == "flying":
            ...
            if self._land_on_surface(platforms):
                ...
                self.despawn_at_ms = pygame.time.get_ticks() + 3000  # 3s after landing

        elif self.state == "splatted_persist":
            # if stepped on by a player, deal 0.5 ONCE and start 3s timer
            if players is not None:
                hit = pygame.sprite.spritecollideany(self, players)
                if hit:
                    self.stepped_on_by(hit)
    ...
    def stepped_on_by(self, player):
        ...
        self.state = "splatted_temp"
        self.despawn_at_ms = pygame.time.get_ticks() + 3000
